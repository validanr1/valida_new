-- Add description column to assessment_types if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'assessment_types' 
    AND column_name = 'description'
  ) THEN
    ALTER TABLE public.assessment_types ADD COLUMN description TEXT;
  END IF;
END $$;

-- Create assessment_type_items table
