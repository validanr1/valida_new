-- Create assessment_types table first (if it doesn't exist)
...
-- Insert default types (GHE and GES)
INSERT INTO public.assessment_types (name, description)
VALUES 
  ('GHE', 'Grupo Homogêneo de Exposição'),
  ('GES', 'Grupo de Exposição Similares')
ON CONFLICT (name) DO NOTHING;

-- Enable RLS on assessment_types
...
-- Grant permissions on assessment_types
GRANT SELECT ON public.assessment_types TO authenticated;

-- Now create assessment_type_items table
...
-- Add updated_at trigger for assessment_types
